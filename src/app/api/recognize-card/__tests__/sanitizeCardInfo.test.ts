 ...
 *   5. Model name (gemini-2.5-pro-preview-05-06) is passed to generateContent
 */
...
describe('POST /api/recognize-card – gap coverage', () => {
  ...
  describe('model configuration', () => {
    it('calls generateContent with model "gemini-2.5-pro-preview-05-06"', async () => {
      genaiMocks.generateContent.mockResolvedValueOnce({
        text: validCardJson(),
      });
      ...
      expect(genaiMocks.generateContent).toHaveBeenCalledWith(
        expect.objectContaining({ model: 'gemini-2.5-pro-preview-05-06' }),
      );
    });
